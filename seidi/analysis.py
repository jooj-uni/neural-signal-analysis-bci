...
piv_sp        = (df.pivot_table(values="score",index="subject",columns="pipeline",aggfunc=np.nanmean)
                 .reindex(index=subjs,columns=pipes))
piv_ep        = (df.pivot_table(values="score",index="session",columns="pipeline",aggfunc=np.nanmean)
...
heatmap(ax_hm_sp, piv_sp.values, pipes, subjs, title="Sujeito × Pipeline (média)",
        xlabel="Pipeline", ylabel="Sujeito")

# ---- (3) Heatmap: Sessão × Pipeline ----
heatmap(ax_hm_ep, piv_ep.values, pipes, sess, title="Sessão × Pipeline (média)",
...
for idx, p in enumerate(pipes):
    ...
    ax_lines.errorbar(sess, y_mean, yerr=1.96 * y_sem, marker='o', capsize=4, label=p)

ax_lines.set_xlabel("Sessão")
ax_lines.set_ylabel("Accuracy (média)")
ax_lines.set_title("Evolução por sessão (média ± IC95%)")
ax_lines.legend(ncols=2, fontsize=8)  # legenda com fonte menor
...
subj_order = (df_best.groupby("subject")["score"].mean()
              .sort_values(ascending=False).index.tolist())
piv_best   = (df_best.pivot_table(values="score", index="subject", columns="session", aggfunc=np.nanmean)
              .reindex(index=subj_order, columns=sess))
heatmap(ax_hm_best, piv_best.values, sess, subj_order,
        title=f"[BEST] {best_pipe}: Sujeito × Sessão", xlabel="Sessão", ylabel="Sujeito")
